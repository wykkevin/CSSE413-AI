      ...
      if not self.walls[nextx][nexty]:
          for i in range(len(self.corners)):
              if (nextx, nexty) == self.corners[i]:
                  new_checks[i] = True          
          nextState = ((nextx, nexty), new_checks)
          cost = self.costFn(nextState)
          successors.append((nextState, action, cost))
    self._expanded += 1
    return successors
...
def cornersHeuristic(state, problem):
  ...
  for i in range(4):
      if not state[1][i]:
          sum =max(sum, abs(state[0][0] - corners[i][0]) + abs(state[0][1] - corners[i][1]))
...
def foodHeuristic(state, problem):
  ...
  ls = foodGrid.asList()
  for i in ls:
#       sum = max(sum, mazeDistance(position, i, problem.startingGameState))
    sum = max(sum, abs(position[0] - i[0]) + abs(position[1] - i[1]))
  return sum
